# 산술연산자
print(10+2)
print(10-2)
print(10*2)
print(10/2)

print(10/10)
print(10//2)
print(10%2)
print(10//3)
print(10%3)

# int() 정수형
# 실수형, 논리형, 문자열

print(int(123.92))
print(int(123.822828))

print(int(True))
print(int(False))

print(int("284242"))
# print(int("1234a")) 잘못된 경우

# float() 실수형
# 정수형, 논리형, 문자열

print(float(200))
print(float(3))

print(float(True))
print(float(False))

print(float("23.23523"))
print(float("2345"))

#str()
# 모든 자료형

print(str(1234))
print(str(3535.3535))
print(str(True))
print(str(False))

print(type(str(1234)))
print(type(str(3535.3535)))
print(type(str(True)))
print(type(str(False)))

#bool
# 모든 자료형

#False
print(bool(0))
print(bool(0.0))
print(bool(""))
#True
print(bool(1))
print(bool(1.0))
print(bool("str"))
print(bool("1234"))

############################
# 비교연산자

# 비교연산자의 종류 a<b , a>b , a<=b , a>=b , a==b , a!=b

a = 10
b = 20

# =(Equal sign) a==b
print(a<b) 
print(a<=b)
print(a>b)
print(a>=b)
print(a==b)
print(a!=b)

# 논리형 자료

is_true = True
is_false = False

print(is_true > is_false) #True
print(is_true > is_false) #False

# 문자형 자료

print("Python" > "python") #False
print("12345" > "12344") # True
print("12.12" < "13.12") # True

# 복합대입연산자
# 산술연산자와 대입연산자를 합쳐서 사용

today =1230
today += 1

print(today)

# 논리연산자
print(10<3)
print(10!=3)

# and

print(True and True)
print(True and False)
print(10>3 and 3<5)
print(10==1 and 1==5)

# or

print(True or False)
print(False or True)
print(False or False)

# and
print("a" and 10>3 and 0)
print("a" and 10>3 and True and False)
print("a" and 10>3 and True and "참")

# or
print (0 or 0.0 or False or "a")

# and or 같이 쓰는 경우
print((True or False)and "참")

# not 연산자
print (not(True))
print(not 10>3)

# 연산자 우선순위
#1. 제곱 2. 양수 음수 3. 곱하기 나누기 4. 덧셈 뺄셈 5. 비교 6. 평등 7. 대입
print (3 * 2 ** 2) # 제곱 먼저 계산 답은 12
print((3 *2)**2)
